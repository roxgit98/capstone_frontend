...
const Admin = () => {
  ...
  const postData = async (e) => {
    ...
    try {
      const url =
        "https://optimistic-dorris-roxgit98-b4431215.koyeb.app/videogiochi";
      const token = localStorage.getItem("authToken");
      const response = await fetch(url, {
        method: "POST",
        headers: {
  ...
  const patchData = async (file) => {
    try {
      const url = `https://optimistic-dorris-roxgit98-b4431215.koyeb.app/videogiochi/${localStorage.getItem(
        "videogiocoId"
      )}/boxArt`;
      const token = localStorage.getItem("authToken");
